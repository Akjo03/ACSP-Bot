...
@Component
@EnableLogger
public class CommandSubcommandsHelpInteractionHandler extends CscButtonInteractionHandler {
	...
	protected CommandSubcommandsHelpInteractionHandler() {
		super(List.of(
				"back:command_subcommands_help",
				"close:command_subcommands_help"
		));
	}
	...
	public void onExecute(@NotNull ButtonInteractionEvent event) {
		...
		switch (event.getComponentId()) {
			case "back:command_subcommands_help" -> helpCommandService.backToCommand(event, helpMessage);
			case "close:command_subcommands_help" -> helpCommandService.closeHelp(event);
		}
	}
}